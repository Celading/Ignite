...
| HTTP/1.1 Client | Preview | `preferTransportBackend("ignite-native-h1-client")` 可显式进入 native H1，支持连接复用、流式请求体、重定向和统一错误；配合 `useNativeTls(...)` 可走 Contract-backed TLS1.3 HTTPS。 | `RestClient` 默认仍是稳定 stdx Client；Native HTTPS 要求调用方提供 trust anchor/hostname，当前不做 TLS 连接池，部分 request hook 仍不兼容。 |
| HTTPS / TLS | 稳定继承 + Preview | 默认继续使用稳定的 stdx TLS 路径；显式 Native H1/H2 TLS1.3 会完成 ClientHello、server-flight 验证、client Finished 与 application-data seal/open。 | Native TLS 当前要求调用方提供 trust anchor/hostname；系统 CA、TLS1.2、TLS 连接池、session resumption、0-RTT、mTLS 与默认切换仍未完成。 |
...
| native HTTP/2 Client | Preview | `RestClient` 显式选择 `ignite-native-h2-client` 后，可通过明文 prior knowledge 使用 native H2；配合 `useNativeTls(...)` 和 `h2` ALPN 可走 Contract-backed TLS1.3。两条路径共享 AP4 HPACK、流控、streamed-response lease、超时、重试、Cookie 与 observe 语义；低层 `NativeH2ClientConnection` 可消费 caller-owned socket 或 `NativeByteTransport`。 | 必须显式选择实验 backend；仅支持 buffered/replayable request body 和每连接一个公开 streamed response lease。明文连接可归池，TLS 连接当前每次请求关闭；mutation hook、默认切换与公开 RestClient 多路并发仍未完成。 |
