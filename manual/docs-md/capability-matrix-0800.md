...
| native HTTP/2 Client | Preview | `RestClient` 显式选择 `ignite-native-h2-client` 后，可通过明文 prior knowledge 使用 native H2；配合 `useNativeTls(...)` 和 `h2` ALPN 可走 Contract-backed TLS1.3。普通请求支持一个公开 streamed-response lease；`sendNativeH2Batch(...)` 另提供最多 32 个同源明文 buffered 请求的单连接多路复用，并保持输入顺序返回。 | 必须显式选择实验 backend。普通请求仍是每连接一个公开 streamed lease；batch 不支持 TLS、streamed response、retry/redirect 重放、逐请求取消或 request/observe/touchpoint hook。任意异步多 lease 与默认切换仍未完成。 |
