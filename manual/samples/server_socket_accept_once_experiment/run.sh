...
OUT_BIN="/tmp/ignite_sample_server_socket_accept_once_experiment"
...
if [[ -z "${STDX_STATIC}" ]]; then
  echo "[accept-once-experiment] cannot locate stdx static path." >&2
  echo "[accept-once-experiment] set IGNITE_STDX_STATIC=/path/to/cj_stdx_*_llvm/static or CANGJIE_STDX_PATH." >&2
  exit 1
fi

RUNTIME_LIB_DIR="$(detect_runtime_lib_dir || true)"
if [[ -z "${RUNTIME_LIB_DIR}" ]]; then
  echo "[accept-once-experiment] cannot locate Cangjie runtime lib dir." >&2
  echo "[accept-once-experiment] set IGNITE_CJ_RUNTIME_LIB_DIR or CANGJIE_HOME." >&2
  exit 1
fi
...
if [[ "${IGNITE_SAMPLE_SKIP_BUILD:-0}" != "1" ]]; then
  echo "[accept-once-experiment] building ignite package..."
  (cd "${ROOT}" && cjpm build)
else
  echo "[accept-once-experiment] skipping ignite package build (IGNITE_SAMPLE_SKIP_BUILD=1)"
fi

echo "[accept-once-experiment] compiling sample..."
cjc "${ROOT}/manual/samples/server_socket_accept_once_experiment/main.cj" \
  "${COMMON_IMPORTS[@]}" \
  "${COMMON_LINKS[@]}" \
  -Woff unused \
  -o "${OUT_BIN}"
...
if [[ "${IGNITE_SAMPLE_COMPILE_ONLY:-0}" == "1" ]]; then
  echo "[accept-once-experiment] compile-only mode; skipping run"
  exit 0
fi
...
echo "[accept-once-experiment] running sample..."
"${OUT_BIN}"
