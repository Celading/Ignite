...
OUT_BIN="/tmp/ignite_sample_server_socket_count_limited_accept_loop_experiment"
...
ensure_cangjie_env() {
  if ! command -v cjpm >/dev/null 2>&1 || ! command -v cjc >/dev/null 2>&1; then
    ...
    elif [[ -f "/Library/Frameworks/Cangjie/1.1.0/envsetup.sh" ]]; then
      envsetup="/Library/Frameworks/Cangjie/1.1.0/envsetup.sh"
...
detect_runtime_lib_dir() {
  ...
  if [[ -n "${CANGJIE_HOME:-}" && -d "${CANGJIE_HOME}/runtime/lib" ]]; then
    ...
    case "$(uname -s)" in
      Darwin)
        hit="$(find "${CANGJIE_HOME}/runtime/lib" -maxdepth 3 -type f -path "*/darwin_*/*" -name "libcangjie-runtime.*" | head -n 1)"
        ;;
      Linux)
        hit="$(find "${CANGJIE_HOME}/runtime/lib" -maxdepth 3 -type f -path "*/linux_*/*" -name "libcangjie-runtime.*" | head -n 1)"
        ;;
      *)
        hit=""
        ;;
...
if [[ -z "${STDX_STATIC}" ]]; then
  echo "[count-limited-accept-loop-experiment] cannot locate stdx static path." >&2
  echo "[count-limited-accept-loop-experiment] set IGNITE_STDX_STATIC=/path/to/cj_stdx_*_llvm/static or CANGJIE_STDX_PATH." >&2
  exit 1
fi

RUNTIME_LIB_DIR="$(detect_runtime_lib_dir || true)"
if [[ -z "${RUNTIME_LIB_DIR}" ]]; then
  echo "[count-limited-accept-loop-experiment] cannot locate Cangjie runtime lib dir." >&2
  echo "[count-limited-accept-loop-experiment] set IGNITE_CJ_RUNTIME_LIB_DIR or CANGJIE_HOME." >&2
  exit 1
fi
...
if [[ "${IGNITE_SAMPLE_SKIP_BUILD:-0}" != "1" ]]; then
  echo "[count-limited-accept-loop-experiment] building ignite package..."
  (cd "${ROOT}" && cjpm build)
else
  echo "[count-limited-accept-loop-experiment] skipping ignite package build (IGNITE_SAMPLE_SKIP_BUILD=1)"
fi

echo "[count-limited-accept-loop-experiment] compiling sample..."
cjc "${ROOT}/manual/samples/server_socket_count_limited_accept_loop_experiment/main.cj" \
  "${COMMON_IMPORTS[@]}" \
  "${COMMON_LINKS[@]}" \
  -Woff unused \
  -o "${OUT_BIN}"
...
if [[ "${IGNITE_SAMPLE_COMPILE_ONLY:-0}" == "1" ]]; then
  echo "[count-limited-accept-loop-experiment] compile-only mode; skipping run"
  exit 0
fi
...
echo "[count-limited-accept-loop-experiment] running sample..."
"${OUT_BIN}"
