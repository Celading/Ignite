...
OUT_BIN="/tmp/ignite_sample_server_socket_keepalive_two_turn_experiment"
...
if [[ -z "${STDX_STATIC}" ]]; then
  echo "[keepalive-two-turn-experiment] cannot locate stdx static path." >&2
  echo "[keepalive-two-turn-experiment] set IGNITE_STDX_STATIC=/path/to/cj_stdx_*_llvm/static or CANGJIE_STDX_PATH." >&2
  exit 1
fi

RUNTIME_LIB_DIR="$(detect_runtime_lib_dir || true)"
if [[ -z "${RUNTIME_LIB_DIR}" ]]; then
  echo "[keepalive-two-turn-experiment] cannot locate Cangjie runtime lib dir." >&2
  echo "[keepalive-two-turn-experiment] set IGNITE_CJ_RUNTIME_LIB_DIR or CANGJIE_HOME." >&2
  exit 1
fi
...
if [[ "${IGNITE_SAMPLE_SKIP_BUILD:-0}" != "1" ]]; then
  echo "[keepalive-two-turn-experiment] building ignite package..."
  (cd "${ROOT}" && cjpm build)
else
  echo "[keepalive-two-turn-experiment] skipping ignite package build (IGNITE_SAMPLE_SKIP_BUILD=1)"
fi

echo "[keepalive-two-turn-experiment] compiling sample..."
cjc "${ROOT}/manual/samples/server_socket_keepalive_two_turn_experiment/main.cj" \
  "${COMMON_IMPORTS[@]}" \
  "${COMMON_LINKS[@]}" \
  -Woff unused \
  -o "${OUT_BIN}"
...
if [[ "${IGNITE_SAMPLE_COMPILE_ONLY:-0}" == "1" ]]; then
  echo "[keepalive-two-turn-experiment] compile-only mode; skipping run"
  exit 0
fi
...
echo "[keepalive-two-turn-experiment] running sample..."
"${OUT_BIN}"
